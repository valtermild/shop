...
router.post('/product/updatecart', (req, res, next) => {
    ...
    async.eachSeries(cartItems, (cartItem, callback) => {
        ...
        if(hasError === false){     
            ...
            if(stockError){
               req.session.cartTotalItems = req.session.cart.reduce((a, b) => +a + +b.quantity, 0);
                res.status(400).json({message: 'There is insufficient stock of this product.', totalCartItems: req.session.cartTotalItems});
            }else{
                req.session.cartTotalItems = req.session.cart.reduce((a, b) => +a + +b.quantity, 0);
                res.status(400).json({message: 'There was an error updating the cart', totalCartItems: req.session.cartTotalItems});
            }
        }
    });
});
...
router.post('/product/removefromcart', (req, res, next) => {
    ...
    async.each(req.session.cart, (item, callback) => {
       ...
       // update total cart amount
        common.updateTotalCartAmount(req, res);        
        res.status(200).json({message: 'Product successfully removed', totalCartItems: req.session.cartTotalItems});              
    });
});
...
router.post('/product/emptycart', (req, res, next) => {
    ...
    delete req.session.cartTotalItems;
