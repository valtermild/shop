...
exports.updateTotalCartAmount = (req, res) => {
    ...
    if(req.session.totalCartAmount < config.freeShippingAmount){        
        req.session.shipping = shippingPrice(parseInt(req.session.cartTotalItems))    
        function shippingPrice(val) {
            return Math.ceil(val / 10) * 10;
        }	
      
        req.session.totalCartAmount = req.session.totalCartAmount + parseInt(req.session.shipping);
